...
export const openPhotoFromRemoteExample = async (): Promise<void> => {
  ...
  // the file you should pass a local URL to the downloaded photo
  // to the editor. This example demonstrates how to achieve the former.
  const remoteURL = "https://img.ly/static/example-assets/LA.jpg";
  ...
  try {
    ...
    const { uri } = await FileSystem.downloadAsync(
      remoteURL,
      FileSystem.documentDirectory + "LA.jpg"
    );
    // highlight-download
    ...
    // There was an error generating/downloading the photo.
    console.log(error);
    // highlight-open-editor
  } finally {
